...
<template>
	<div class="single-comment-body child" v-for="reply in replies_all">
		...
		<div class="comment-text-body" :data-comment-id="reply.id">
			<h4>{{ reply.author.name }} 
				<span class="comment-date">{{ reply.date }}</span> 
				<a @click='replyTo(reply.author.name, reply.id)' class="reply-to">reply</a>
			</h4>
			<p>{{ reply.descr }}</p>
			<!-- <a class="show-reply" 
			...
			<a class="show-reply"
				v-show="reply.has_replies"
				@click='showReplies($event, reply.id)'>Show Replies</a>
